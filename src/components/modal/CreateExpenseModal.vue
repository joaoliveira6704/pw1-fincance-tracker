...
export default {
  ...
  computed: {
    ...
    textareaClass() {
      return "w-full bg-[#9e9e9e]/50 rounded-xl px-3 py-2 hover:bg-[#C5C4CB]/50 focus:bg-[#C5C4CB]/50 transition-colors outline-1 outline-gray-500 focus:outline-1 resize-none";
    },
  },
  methods: {
    ...
    async createExpense() {
      ...
      if (!this.categoryId || !this.name || !this.amount || !this.date) {
      ...
      try {
        ...
        console.error(e);
        this.error = "Ocorreu um erro ao criar a despesa.";
        toast.fire({
          icon: "error",
    ...
    <Transition name="fade">
      ...
      >
        ...
        >
          ...
          <form @submit.prevent="createExpense" class="p-8 pt-6 space-y-6">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div class="flex flex-col gap-y-1 p-2">
                <label class="ml-1 font-Roboto text-primary-text"
                  >Categoria :</label
                >

                <select v-model="categoryId" :class="categoryClass">
            ...
            <div class="px-2 pt-2">
              <button
                type="submit"
                class="w-full flex justify-center py-3 px-4 rounded-lg text-stackrblack bg-stackrgreen-500 hover:bg-stackrgreen-200 font-bold font-ProximaNova transition-all shadow-md transform hover:scale-[1.01]"
              >
